...
<%
    ...
    List<DeliveryAssignment> acceptedOrders = new ArrayList<>();
    ...
    try {
        ...
        for (DeliveryAssignment da : all) {
            if ("PICKED_UP".equals(da.getStatus()) || "IN_TRANSIT".equals(da.getStatus())) {
                acceptedOrders.add(da);
            }
        }
    } catch (Exception e) {
...
.btn-complete { padding:6px 14px; background:#10b981; color:#fff; border:none; border-radius:var(--radius-md); cursor:pointer; font-size:0.85em; font-weight:600; }
...
<main class="container">
    ...
    <div class="section-card">
        <h3>Active Deliveries (<%= acceptedOrders.size() %>)</h3>
        <% if (acceptedOrders.isEmpty()) { %>
            <p class="empty-msg">No active deliveries at the moment.</p>
        <% } else { %>
        <table class="assignment-table">
            ...
            <tbody>
                <% for (DeliveryAssignment da : acceptedOrders) { %>
                <tr>
                    <td><%= da.getOrderId() %></td>
                    ...
                    <td><%= da.getPickedUpAt() != null ? da.getPickedUpAt().toString().substring(0, 16) : "N/A" %></td>
                    <td><span class="badge badge-picked"><%= da.getStatus() %></span></td>
                    <td>
                        <button class="btn-complete" onclick="completeOrder(<%= da.getAssignmentId() %>, this)">Mark Delivered</button>
                    </td>
                </tr>
                <% } %>
            </tbody>
...
<script>
function completeOrder(assignmentId, btn) {
    if (!confirm('Mark this order as delivered?')) return;
    btn.disabled = true;
    fetch('${pageContext.request.contextPath}/deliveryAssignment', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'action=deliver&assignmentId=' + assignmentId
    })
    .then(r => r.json())
    .then(data => {
        if (data.success) {
