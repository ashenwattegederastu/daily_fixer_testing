...
<body>
    <div style="max-width: 1200px; margin: 2rem auto; padding: 0 1rem;">
        <h1 style="font-size: 2rem; font-weight: 700; margin-bottom: 1rem; color: var(--foreground);">My Chats</h1>
                ...
                <a href="${pageContext.request.contextPath}/chats/view?chatId=${chat.chatId}" 
                   style="background: var(--card); border-radius: var(--radius); padding: 1.5rem; box-shadow: var(--shadow-sm); border: 1px solid var(--border); text-decoration: none; display: block; transition: all 0.2s;">
                    <div style="display: grid; grid-template-columns: 1fr auto; gap: 1rem; align-items: center;">
                        <div>
                            <h3 style="font-size: 1.125rem; font-weight: 600; margin-bottom: 0.25rem; color: var(--foreground);">
                                <c:choose>
                            ...
                            <p style="color: var(--muted-foreground); font-size: 0.875rem; margin-bottom: 0.5rem;">${chat.serviceName}</p>
                            <c:if test="${not empty chat.lastMessage}">
                                <p style="color: var(--muted-foreground); font-size: 0.875rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                    ${chat.lastMessage}
                                </p>
                            </c:if>
                        </div>
                        <div style="text-align: right;">
                            <c:if test="${chat.unreadCount > 0}">
                                <span style="display: inline-block; background: var(--destructive); color: var(--destructive-foreground); padding: 0.25rem 0.5rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 600;">
                                    ${chat.unreadCount}
                                </span>
                            </c:if>
                        </div>
